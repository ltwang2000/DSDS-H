...
from fairseq.modules.my_transformer_layer import MyTransformerEncoderLayer
...
EncoderOut = namedtuple(
    ...
    [
        ...
        "src_tokens",           # B x T
        "src_lengths",          # B
        #"src_text_repr",        # B x D (用于对齐损失)
        #"vis_repr",             # B x D (用于对齐损失)
    ],
)
print("==== MyTransformerModel module is loaded ====")  # 添加在文件顶部
@register_model("my_transformer_base")
...
class MyTransformerEncoder(FairseqEncoder):
    ...
    def forward_embedding(
        ...
        return x, embed
    """
    def pool_representations(self, text_feats, text_padding_mask, visual_feats):
        池化文本和视觉特征，得到句子级别的表示。
        text_feats_b = text_feats.transpose(0, 1)
        mask = ~text_padding_mask.unsqueeze(-1)
        pooled_text = (text_feats_b * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1e-9)
        pooled_visual = visual_feats.mean(dim=1)
        return pooled_text, pooled_visual
    """
    def forward(
            self,
            src_tokens,
        ...
        grid_feats_proj = self.grid_projection(grid_img_features) + self.grid_pos_embed
        region_feats_proj = self.region_projection(region_img_features) + self.region_pos_embed
        grid_img_mask = (grid_feats_proj.norm(p=2, dim=-1) < 1e-5)
        region_img_mask = (region_feats_proj.norm(p=2, dim=-1) < 1e-5)

        # --- MODIFICATION START: Corrected Loop and Return Logic ---

        # 3. Encoder Layer Loop
        final_selected_visuals = None
        encoder_states = [] if return_all_hiddens else None

        for idx, layer in enumerate(self.layers):
            output = layer(
                x, grid_feats_proj, region_feats_proj,
                encoder_padding_mask, grid_img_mask, region_img_mask,
                src_tokens.size(0), idx
            )

            if isinstance(output, tuple):
                x, layer_visuals = output
                if idx == self.num_layers - 1 and layer_visuals is not None:
                    final_selected_visuals = layer_visuals
            else:
                x = output
        ...
        #if final_selected_visuals is None:
        #    final_selected_visuals = region_feats_proj

        #src_text_repr, vis_repr = self.pool_representations(
        #    text_feats=x,
        #    text_padding_mask=encoder_padding_mask,
        #    visual_feats=final_selected_visuals
        #)
        ...
        return EncoderOut(
            ...
            src_tokens=src_tokens,
            src_lengths=src_lengths,
            #src_text_repr=src_text_repr,
            #vis_repr=vis_repr,
        )

    @torch.jit.export
    def reorder_encoder_out(self, encoder_out: EncoderOut, new_order: torch.Tensor):
        """
        """
        # ---- 标准字段处理 ----
        new_encoder_out = (
        ...
        new_src_lengths = (
            encoder_out.src_lengths.index_select(0, new_order)
            if encoder_out.src_lengths is not None
            else None
        )

        new_encoder_states = (
            [state.index_select(1, new_order) for state in encoder_out.encoder_states]
        ...
        """
        new_src_text_repr = (
            encoder_out.src_text_repr.index_select(0, new_order)
            if encoder_out.src_text_repr is not None
            else None
        )

        new_vis_repr = (
            encoder_out.vis_repr.index_select(0, new_order)
            if encoder_out.vis_repr is not None
            else None
        )
        """
        # 返回重构的EncoderOut对象
        return EncoderOut(
            ...
            #src_text_repr=new_src_text_repr,
            #vis_repr=new_vis_repr,
        )
...
@register_model_architecture("my_transformer_base", "my_transformer_def")
...
@register_model_architecture("my_transformer_base", "my_transformer")
def my_transformer(args):
    ...
    #args.alignment_weight = getattr(args, "alignment_weight", 0.1)
    base_architecture(args)
